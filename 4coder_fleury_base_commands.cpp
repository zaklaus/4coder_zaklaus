...
zpl_push_build_directory_at_file(Application_Links *app, Arena *arena, Buffer_ID buffer){
...
global String_Const_u8 zpl_build_file_name_array[] = {
    str8_lit("build.bat"),
};
global String_Const_u8 zpl_build_cmd_string_array[] = {
    str8_lit("build"),
};
...
static String_Const_u8
zpl_push_fallback_command(Arena *arena, String_Const_u8 file_name){
    return(push_u8_stringf(arena, "echo could not find %.*s", string_expand(file_name)));
}

static String_Const_u8
zpl_push_fallback_command(Arena *arena){
    return(zpl_push_fallback_command(arena, zpl_build_file_name_array[0]));
}

global_const Buffer_Identifier zpl_build_build_buffer_identifier = buffer_identifier(string_u8_litexpr("*compilation*"));

global_const u32 zpl_build_exec_flags = CLI_OverlapWithConflict|CLI_SendEndSignal|CLI_CursorAtEnd;

static void
zpl_build_exec_command(Application_Links *app, View_ID view, String_Const_u8 dir, String_Const_u8 cmd){
    exec_system_command(app, view, zpl_build_build_buffer_identifier,
                        dir, cmd,
                        zpl_build_exec_flags);
}

function b32
zpl_search_and_build_from_dir(Application_Links *app, View_ID view, String_Const_u8 start_dir){
    Scratch_Block scratch(app);
    
    // NOTE(allen): Search
    String_Const_u8 full_file_path = {};
    String_Const_u8 cmd_string  = {};
    for (i32 i = 0; i < ArrayCount(zpl_build_file_name_array); i += 1){
        full_file_path = push_file_search_up_path(app, scratch, start_dir, zpl_build_file_name_array[i]);
        if (full_file_path.size > 0){
            cmd_string = zpl_build_cmd_string_array[i];
            break;
        }
    }
    ...
    if (result){
        ...
        zpl_build_exec_command(app, view, path, command);
...
static void
zpl_search_and_build(Application_Links *app, View_ID view, Buffer_ID active_buffer){
    Scratch_Block scratch(app);
    b32 did_build = false;
    String_Const_u8 build_dir = zpl_push_build_directory_at_file(app, scratch, active_buffer);
    if (build_dir.size > 0){
        did_build = zpl_search_and_build_from_dir(app, view, build_dir);
    }
    if (!did_build){
    ...
    if (!did_build){
        zpl_build_exec_command(app, view,
                                    push_hot_directory(app, scratch),
                                    zpl_push_fallback_command(scratch));
    }
}


CUSTOM_COMMAND_SIG(zpl_build_in_build_panel)
CUSTOM_DOC("Looks for a build.bat, build.sh, or makefile in the current and parent directories.  Runs the first that it finds and prints the output to *compilation*.  Puts the *compilation* buffer in a panel at the footer of the current view.")
{
    ...
    zpl_search_and_build(app, build_view, buffer);
...
CUSTOM_COMMAND_SIG(zaklaus_execute_previous_cli)
...
{
    String_Const_u8 out_buffer = SCu8("*console*");
...
CUSTOM_COMMAND_SIG(zaklaus_execute_any_cli)
CUSTOM_DOC("Queries for an output buffer name and system command, runs the system command as a CLI and prints the output to the specified buffer."){
    ...
    zaklaus_execute_previous_cli(app);
}
